...
**Q2_New** Which U.S. state saw the highest percentage increase in opiod pills from 2006 to 2014?

--Produce two tables, one for 2006 and one for 2014. In the 2006 table, rename the total to total_2006. In the 2014 table, rename the total to total_2014. --Join the tables into a new table, arcos_combo --Your table must have the full name of the states.

Join with the state crosswalk table --Calculate the percentage change between 2006 and 2014 --Create one table with the top 5 states by percentage increase --Create a second table with the 5 states with the lowest percentage increase.

```{r}
#load state crosswalk and state data 
state_population <- read_csv("~/Documents/Github/natalie_weger_jour472/lab_07/data/state_population_2014.csv")

state_crosswalk <- read_csv("~/Documents/Github/natalie_weger_jour472/lab_07/data/state_crosswalk.csv")

#creating a 2006 and 2014 table 
total_2006 <- arcos %>%
  filter(year==2006)
total_2014 <- arcos %>%
  filter(year==2014)

#creating an acros_combo table and arcos_combo_sidewalk table 
...
#grouping the amount of pills by county to states
arcos_state <- arcos %>%
  left_join (state_crosswalk, by = c("buyer_state" = "state_abb")) %>%
  group_by(state_name) %>%
  summarise(total_2006 = sum(total_pills[year == 2006]),
            total_2014 = sum(total_pills[year == 2014]))

#making percentage change column by state  
arcos_state <- arcos_state %>%
    mutate(percent_change=((total_2014-total_2006)/total_2006)*100)


#creating two tables for highest and lowest percentage change amount pills in each state
top_states <- arcos_state %>%
  arrange(desc(percent_change)) %>%
  slice_max(percent_change, n = 5)
  

bottom_states <- arcos_state %>%
  arrange(desc(percent_change)) %>%
