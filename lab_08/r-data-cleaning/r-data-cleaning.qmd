...
**Answer:** The method from this lab that I would use to group and summarize to determine the total population for the state is using "mutate" and "case_when." I would also make a new dataset and call it something like "maryland_clean" that could take different values such as "Maryland" "md" and "MD" and recognize that if these values are TRUE, then they can be assigned their population value to a new population column for that row. This can leave the original data as is but will allow for a new dataset that can recognize Maryland as just one state. I can then add up all of the population values from my new column to determine the population across counties in Maryland.
